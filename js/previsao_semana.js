...
//         }
//     }
//     Httpreq.open("GET", `https://viacep.com.br/ws/${cep}/json/`, true);
//     Httpreq.send()
...
async function fetchGeo(cidade, estado) {
    const responseGeolocation = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${cidade},${estado},076&appid=38fe5f0e298f3edf79048384cd436a89`);
    ...
    const responseWeather = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=e9093dcdb40d47a6b46160842220604&q=${lat},${lon}&days=7&aqi=yes&lang=pt`)
...
function criaCardAgora(hoje) {
    ...
    iconImagem.setAttribute('src', `img/64x64/${diaOuNoite}/${hoje.icone.slice(-7, -4)}.png`)
    agoraCard.appendChild(iconImagem)

    /*TEMPERATURA AGORA*/
    agoraTemperatura.innerHTML = `<p>temperatura</p><span>${hoje.temperatura.toFixed(1)}ºC</span>`;
...
function criaCardPrevisoes(dias) {
    ...
    dias.forEach(dia => {
        ...
        previsaoTitulo.innerHTML = `<p>previsões para ${dia.data.slice(8, 10)}/${dia.data.slice(5, 7)}</p>`;
